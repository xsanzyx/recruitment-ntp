...
<header class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h1 class="text-2xl sm:text-3xl font-extrabold text-[#002870] tracking-tight">Edit Lowongan</h1>
        <p class="text-gray-500 text-sm mt-1">Perbarui informasi lowongan: <strong class="text-gray-700">{{ $vacancy->title }}</strong></p>
    </div>
...
<form method="POST" action="{{ route('hr.vacancies.update', $vacancy->id) }}" class="space-y-8 max-w-4xl">
    @csrf @method('PUT')
        ...
        <div class="p-6 sm:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" id="title" class="w-full h-11 px-4 bg-slate-50 border {{ $errors->has('title') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-sm font-medium outline-none focus:border-[#002870] focus:ring-2 focus:ring-[#002870]/10 transition-all" value="{{ old('title', $vacancy->title) }}">
                    @error('title')
                ...
                <div>
                    ...
                    <input type="text" name="department" id="department" class="w-full h-11 px-4 bg-slate-50 border {{ $errors->has('department') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-sm font-medium outline-none focus:border-[#002870] focus:ring-2 focus:ring-[#002870]/10 transition-all" value="{{ old('department', $vacancy->department) }}">
                    @error('department')
                        <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="division" id="division" class="w-full h-11 px-4 bg-slate-50 border {{ $errors->has('division') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-sm font-medium outline-none focus:border-[#002870] focus:ring-2 focus:ring-[#002870]/10 transition-all" value="{{ old('division', $vacancy->division) }}">
                    @error('division')
                        <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="deadline" id="deadline" class="w-full h-11 px-4 bg-slate-50 border {{ $errors->has('deadline') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-sm font-medium outline-none focus:border-[#002870] focus:ring-2 focus:ring-[#002870]/10 transition-all" value="{{ old('deadline', $vacancy->deadline->format('Y-m-d')) }}" min="{{ date('Y-m-d', strtotime('+1 day')) }}">
                    @error('deadline')
                        <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-6 sm:p-8 space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5" class="w-full px-4 py-3 bg-slate-50 border {{ $errors->has('description') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-sm outline-none resize-none focus:border-[#002870] focus:ring-2 focus:ring-[#002870]/10 transition-all">{{ old('description', $vacancy->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="requirements" id="requirements" rows="5" class="w-full px-4 py-3 bg-slate-50 border {{ $errors->has('requirements') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-sm outline-none resize-none focus:border-[#002870] focus:ring-2 focus:ring-[#002870]/10 transition-all">{{ old('requirements', $vacancy->requirements) }}</textarea>
                @error('requirements')
                    <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="p-6 sm:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                    ...
                    <input type="number" name="min_age" id="min_age" min="15" max="65" class="w-full h-11 px-4 bg-slate-50 border {{ $errors->has('min_age') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-sm font-medium outline-none focus:border-[#002870] focus:ring-2 focus:ring-[#002870]/10 transition-all" value="{{ old('min_age', $vacancy->min_age) }}" placeholder="cth: 18">
                    @error('min_age')
                        <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="max_age" id="max_age" min="15" max="65" class="w-full h-11 px-4 bg-slate-50 border {{ $errors->has('max_age') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-sm font-medium outline-none focus:border-[#002870] focus:ring-2 focus:ring-[#002870]/10 transition-all" value="{{ old('max_age', $vacancy->max_age) }}" placeholder="cth: 35">
                    @error('max_age')
                        <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div class="flex justify-end gap-3">
        ...
        <button type="submit"
            ...
            <span class="material-symbols-outlined" style="font-size:16px;">check_circle</span> Simpan Perubahan
        </button>
    </div>
