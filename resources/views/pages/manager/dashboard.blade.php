...
<div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
    <div>
        <p class="text-[11px] font-bold text-[#f8b830] uppercase tracking-[.15em] mb-1">Manager Panel</p>
        <h1 class="text-[28px] font-extrabold text-[#001544] tracking-tight leading-tight">Dashboard Manager</h1>
        <p class="text-gray-500 text-[15px] mt-1">Pantau data pelamar di departemen <strong>{{ $department }}</strong>.</p>
    </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
    ...
    <div class="bg-white p-5 rounded-xl border border-gray-100 hover:shadow-md transition-all group">
        ...
        <div class="flex items-center gap-1.5">
            ...
            <span class="text-[12px] text-gray-400 font-medium">Di departemen {{ $department }}</span>
        </div>
    </div>
    ...
    <div class="bg-white p-5 rounded-xl border border-gray-100 hover:shadow-md transition-all group">
        ...
        <div class="flex items-center gap-1.5">
            ...
            @else
                ...
                <span class="text-[12px] text-gray-400 font-medium">Menunggu review HR</span>
            @endif
        </div>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    {{-- LEFT: Lowongan di Departemen --}}
    <div class="bg-white border border-gray-100 rounded-xl overflow-hidden flex flex-col">
        <div class="px-6 py-4 border-b border-gray-50 flex justify-between items-center">
            <h3 class="text-[16px] font-bold text-[#001544]">Lowongan Departemen {{ $department }}</h3>
        </div>
        ...
        <div class="flex-1 p-4 space-y-1">
            @forelse($topVacancies as $index => $vacancy)
                ...
                @endphp
                <div class="flex items-center gap-4 p-4 rounded-xl hover:bg-gray-50 transition-all group">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[12px] text-gray-400 mt-0.5">
                            {{ $vacancy->division ?? $vacancy->department }}
                        </p>
                    </div>
            ...
            @empty
                <div class="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p class="text-sm text-gray-400">Belum ada lowongan di departemen ini.</p>
                </div>
            @endforelse
        </div>
    </div>

    {{-- RIGHT: Pelamar Terbaru --}}
    <div class="bg-white border border-gray-100 rounded-xl overflow-hidden flex flex-col">
        <div class="px-6 py-4 border-b border-gray-50 flex justify-between items-center">
            <h3 class="text-[16px] font-bold text-[#001544]">Pelamar Terbaru</h3>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center">
                                ...
                                <p class="text-sm text-gray-400">Belum ada pelamar terbaru</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div class="bg-[#001544] rounded-2xl p-8 md:p-10 relative overflow-hidden">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 relative z-10">
        <div class="max-w-lg">
            <h4 class="text-[22px] font-extrabold text-white mb-2">Panel Monitoring Pelamar</h4>
            <p class="text-blue-200/80 text-[14px] leading-relaxed">
                Anda dapat memantau seluruh pelamar di departemen <strong class="text-white">{{ $department }}</strong> secara real-time.
                Untuk mengubah status pelamar, silakan hubungi tim HR.
            </p>
        </div>
        <a href="{{ route('manager.applications.index') }}"
    ...
    <span class="material-symbols-outlined absolute -right-8 -bottom-8 text-white/[0.03]"
          style="font-size:220px; font-variation-settings:'FILL' 1;">monitoring</span>
</div>
