...
<section style="padding: 100px 0 80px;">
    <div class="container" style="max-width: 780px;">
        ...
        <form id="profile-form" action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="fade-up d-flex justify-content-end gap-3">
                <a href="{{ url()->previous() }}" class="btn px-4 py-2" style="border-radius:10px;border:1px #e5e7eb;color:#64748b;">Batal</a>
                @php $hasErrors = $errors->any(); @endphp
                <button id="btn-save-profile" type="submit"
                    class="btn {{ $hasErrors ? 'btn-secondary-custom' : '' }} px-4 py-2"
                    style="border-radius:10px; transition:all 0.3s; {{ $hasErrors ? '' : 'border:1px #e5e7eb; color:#64748b; background:transparent;' }}"
                    {{ $hasErrors ? '' : 'disabled' }}>
    ...
    {{-- Modal Crop --}}
    <div id="crop-modal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:99999;overflow-y:auto;">
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:480px;width:90%;margin:5vh auto;position:relative;">
